...
# 版本：v4.0 | 更新：2026-04-01（含修订文档全部更正）
...
- 完整模拟 NCCN 标准诊疗闭环：Workup → 临床分期 → 初始治疗 → 手术 → 病理检查 → 辅助治疗
- 五步顺序推理，上下文累积，错误可级联传播
- 覆盖：口腔癌、口咽癌(p16-)、口咽癌(p16+)、下咽癌、声门喉癌、声门上喉癌

**两种模型角色**：
- **被测模型（Candidate MLLM）**：接收输入，输出严格 JSON，走完五步推理链
- **评判模型（Judge LLM）**：评判自由文本字段（evidence、reasoning），用预定义标准逐条 T/F
...
**重要限制与预处理注意**：
- Hancock **无 CT/MRI**，`describe_focus` 需专家合成或脚本生成后专家审核
- 淋巴结 WSI 缺失 367 例，需分类处理（见第4节）
- Hancock 将声门喉癌与声门上喉癌**均归为喉癌**，需额外根据 ICD codes 区分，由 AI 归类后专家检查
- 部分病史/手术报告可能透露 cT/cN 信息，需标注 `contains_cTcN_leak: true` 并**删去泄露内容**
...
### A级：必须介入确保数据准确（可AI辅助生成初稿，专家检查完善）

**① describe_focus（影像学描述）— Step 1 核心输入**
- Hancock 无 CT/MRI，需根据 pT+pN+tumor_site 合成影像学描述
- **修订方法**：用脚本/大模型按 T/N 分期模板批量生成初步样例，经专家检查后完善（不再是纯手工）
- 额外：标注 `contains_cTcN_leak: true/false`，泄露内容同步删除

**② NCCN 路径节点（`nccn_pathway_node`）— Step 2 GT**
- 规则引擎按 tumor_site + cT + cN + p16 自动定位节点，专家抽样验证20%，**已经人工整理完毕demo**,各个case需程序辅助完成
- 重点审核：T3/T4a 边界、声门上喉癌 N+ 分流
- **注意**：部分节点推荐等级不存在（如 OR-2、OR-3 无固定等级），不可自行填充，见第6节映射表

**③ 口咽癌 p16 状态处理**
- 对所有患者**均不在输入中提供** `hpv_association_p16` 字段的阴/阳性结果
- Step 1→Step 2 之间设额外问题：询问下一步操作时，被测模型**必须回答"需要 p16 结果才能进入 Step 2"**
- 此额外回答需单独设计评分逻辑，属于 Step 1 的附加子任务
- **例外**：531号口咽癌患者未测试 p16 且 ground truth 较少，**建议直接剔除**

**④ 声门喉癌 vs 声门上喉癌区分**
- 原数据集将两者均归为"喉癌"
- 映射时根据 ICD codes 由 AI 自动归类，专家检查确认
- 这两类病种使用完全不同的 NCCN 路径（GLOT vs SUPRA），区分正确与否直接影响 GT

**⑤ 指南偏差案例（两类）**
- **类型A**：指南应辅助治疗但数据显示未做（由于现实原因） → 标注 `patient_declined: true` + `guideline_recommends`
- **类型B**：数据矛盾（如"辅助系统疗法:no" 但"模式:氟尿嘧啶+顺铂"）→ 修正矛盾，标注正确GT
- 可 AI 辅助筛选出这两类，专家最终确认
...
| `high_risk_gross_signals` | 关键词提取 | 与 perinodal 对应关系 |
| `adjuvant_decision_trigger` | 从病理特征自动生成 | 多特征并存优先级 |
| ICD codes 分类（声门/声门上） | AI归类 | 全量专家检查 |
...
Case_XXXX/
├── meta.json
├── step1_input.json        # Workup（不含p16结果）
├── step1_gt.json           # cT/cN GT + p16子任务GT
├── step2_gt.json           # NCCN路径GT
├── step3_input.json        # 手术报告+病史（已删辅助治疗建议）
...
├── step4_gt.json           # 微观病理GT（pathological_data.json）
└── step5_gt.json           # 辅助治疗GT（含指南偏差修正）
```
...
| 情况 | 标记 | 模型应输出 | 评分 |
|------|------|-----------|------|
| pN0（无转移） | `[LN_WSI_NOT_AVAILABLE — pN0]` | `"not_applicable"` | 输出not_applicable得满分 |
| pN+但WSI不可得 | `[LN_WSI_NOT_AVAILABLE — specimen_unavailable]` | `"Cannot Determine"` | Cannot Determine满分；输出yes/no扣分+CSS惩罚 |
| `closest_margin`无法从图像测量 | Step5前系统补充提供 | 使用系统提供值并注明来源 | 正常评分 |
...
**阶段**：`PRIMARY_TREATMENT_DECISION`

**输入**：Step 1 模型输出 + （口咽癌情况下）p16结果
...
```json
{
  ...
  //"nccn_pathway_reasoning": "推演逻辑",（可考虑是否检查）
  "recommended_treatment_options": ["..."],
  "surgery_detail": "具体手术内容",
  "recommendation_category": "2A类",//无则填为NULL
  //"next_action": "SURGERY_EXECUTION"(由于有)
}
```
...
**零幻觉硬约束**：evidence 必须直接引用手术报告原文；完全未提及的填 `"Not Available"`

**必须输出**（未输出不给分）：
- `histologic_type` + evidence
...
**pN 处理**：若报告提及转移可输出 `"pN+"`，不要求精确分期，精确分期由 Step 4 完成
...
**必须输出**（全部字段）：
- 更新分期：`updated_pT_stage`, `updated_pN_stage`, `updated_resection_status`, `updated_histologic_type`
- 微观侵袭：`perineural_invasion_Pn`, `lymphovascular_invasion_L`, `vascular_invasion_V`
- 形态分级：`grading_hpv`, `carcinoma_in_situ`, `resection_status_carcinoma_in_situ`
- 测量：`infiltration_depth_in_mm`（必须从比例尺测量）, `closest_resection_margin_in_cm`
- 淋巴结：`perinodal_invasion`, `pN_stage`（精确）

### Step 5：最终病理汇总 → 辅助治疗决策
**阶段**：`ADJUVANT_TREATMENT_DECISION`

**两个子任务**：
- **子任务A（20%）**：汇总 Steps 1-4 全部信息，输出完整患者画像//也可考虑是否添加
...
**升期路径切换（融贯性核心测试）**：
- Step 4 pT > Step 1 cT → Step 5 必须：说明升期原因 + 重新定位NCCN节点 + 将升期列为辅助治疗依据
- 任何缺失 → `[CASCADE_ERROR]` 惩罚

---

## 6. NCCN 路径完整映射表（Step 2 GT 规则引擎，已人工修订）
根据2026新版NCCN指南编写，之后应考虑指南更新可能。
...
**OR-3**：
- 手术（首选）；拒绝则转ADV
- 手术内容：N0/N1/N2a-b/N3：原发灶切除+/-同侧或双侧颈清
N2c：原发灶切除+/-同侧或双侧颈清
...
**ORPHPV-4**：
- 同步全身治疗/放疗（首选）；手术；诱导化疗（3类），随后进行放疗或同步全身治疗/放疗；临床试验
- 手术内容：原发灶切除+同侧或双侧颈清
...
**HYPO-3**：
- 放疗或同步全身治疗/放疗；手术；诱导化疗；临床试验
- 手术内容：部分或全喉下咽切除+同侧或双侧颈清±甲状腺切除+气管前和同侧气管旁淋巴结清扫

**HYPO-5**：
- 手术；诱导化疗（3类）；同步全身治疗/放疗（3类）；临床试验
...
**GLOT-3**：
- 同步全身治疗/放疗（不适合全身治疗时单纯RT）；手术；诱导化疗；临床试验
- 手术内容：同侧或双侧颈清 + 考虑甲状腺切除以清除中央区淋巴结

**GLOT-4**：
- 同步全身治疗/放疗；手术；诱导化疗；临床试验
- 手术内容：根据指征行喉切除联合甲状腺切除 +同侧或双侧颈清+气管前和同侧气管旁淋巴结清扫

**GLOT-6**：
- T4a,N0-3：手术；拒绝或入选T4a患者则考虑放化疗/临床试验/诱导化疗
...
**SUPRA-2**：
- 手术（适合保留喉功能者：大多数 T1–2, N0 期；选定的 T3 期患者）；RT
- 手术内容：内镜或开放部分喉切除+颈清

**SUPRA-3**：
- 同步全身治疗/放疗或单纯RT（患者身体不适合时）；手术；诱导化疗；临床试验
- 手术内容：喉切除术+甲状腺切除术+同侧、中央组或双侧颈清扫术

**SUPRA-5**：
- 同步放化疗或RT或根治性放疗（肿瘤负荷较低T1-2,N1或不适合全身治疗）；手术；诱导化疗；临床试验
- 手术内容：内镜或开放部分喉切除+颈清
...
**SUPRA-8**：
- 手术（T4a,N0-N3首选）；拒绝则考虑放化疗/临床试验/诱导化疗
- 手术内容：内镜或开放部分喉切除+颈部淋巴结清扫术

---

## 7. 辅助治疗完整决策树（Step 5 GT，已修订）
...
Step5：20%×汇总子任务 + 80%×决策子任务
Step1口咽癌附加：p16子任务独立评分（不计入总分，单独报告）
...
| 错误 | Flag | 惩罚 |
|------|------|------|
| 漏报perinodal | [PERINODAL_MISS] | Step4×0.3；CSS-0.30 |
| 漏开放化疗 | [CHEMORT_MISSED] | Step5决策×0.2；CSS-0.30 |
...
| 情况 | meta标记 | 处理 |
|------|---------|------|
| 非手术首选 | `is_non_surgical: true` | Step2后直接跳Step5，Step3/4标记NA |
...
*文档结束 | 下次继续时将此文件提供给 Claude，说明当前进展到哪个 TODO 项*
