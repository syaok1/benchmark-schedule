...
# 版本：v1.0 | 2026-04-01
# 读取顺序：先读 hnc-benchmark-context-v4.md，再读本文件
...
[ ] 解析 clinical_data.json + pathological_data.json
    输出：每个患者的字段完整性报告
    重点检查字段：pT_stage, pN_stage, resection_status, perinodal_invasion,
...
[ ] 声门喉癌 vs 声门上喉癌区分脚本
    输入：icd_codes 字段
    ICD规则：C32.0=声门喉癌(Glottic)；C32.1=声门上喉癌(Supraglottic)；C32.2=声门下；C32.9=喉癌NOS
...
[ ] 编写 NCCN 路径规则引擎（nccn_router.py）
    ...
    规则表（严格按照 context-v4.md 第6节，不得自行修改）：
...
[ ] 辅助治疗决策规则引擎（adjuvant_router.py）
    ...
    规则严格按照 context-v4.md 第7节各类型决策流
```

### P0-4 describe_focus 生成
```
[ ] 编写 describe_focus 生成脚本（generate_describe_focus.py）
    方法：调用 Claude API，按 tumor_site + cT + cN + histologic_type 生成影像学描述
    
    Prompt 模板（示例）：
    "你是放射科专家。根据以下病理信息，生成一段标准化的影像学描述（CT或超声），
    描述应只包含支持该分期的影像学征象，不得透露pT/pN字样（只用影像描述）。
    肿瘤部位：{tumor_site}，临床T分期：{cT}，临床N分期：{cN}，组织类型：{histologic_type}。
    100-150字，中文，仅输出描述文本。"
    
    输出：describe_focus字段写入 step1_input.json
    标记字段：describe_focus_source: "ai_generated_pending_review"
    
    注意：生成后不自动使用，必须经专家检查后将标记改为 "expert_reviewed"

[ ] 泄露信息检测脚本（detect_cTcN_leak.py）
...
[ ] 编写 case_builder.py
    ...
    step1_input.json 字段：
      ...
      describe_focus（AI生成待审核）, medical_history
      【不包含】hpv_association_p16, adjuvant_treatment_intent
    
    step1_gt.json：
      current_phase: "CLINICAL_STAGING"
      cT_stage（来自pathological_data.json的cT字段）
      cT_evidence_anchor（AI提取待审核）
      cN_stage, cN_evidence_anchor
      p16_subask_required: true/false（口咽癌=true）
    
    step2_gt.json：
      nccn_pathway_node（规则引擎自动生成）
      recommendation_category（按节点填写，无等级的节点不填）
    ...
    step4_gt.json（直接来自 pathological_data.json）：
...
[ ] 生成专家标注工作表（expert_review_tasks.xlsx 或 .csv）
    包含以下待审核项：
    - describe_focus 审核列表（AI生成，需确认或修改）
    - evidence anchor 审核列表（AI提取，需确认）
    - 泄露信息删除列表（需手动删除）
    - 矛盾数据修正列表（类型A/B）
...
[ ] structured_scorer.py
    实现所有结构化字段的自动评分（按 context-v4.md 第8节规则）
...
[ ] 排行榜设计
    列：模型名 | 总分 | Step1 | Step2 | Step3 | Step4 | Step5 | CSS | AR/IND | 测评日期
...
hnc-benchmark/
├── README.md
├── hnc-benchmark-context-v4.md       # 本项目上下文（Claude读取用）
├── hnc-benchmark-todo.md             # 本TODO文件
│
...
│   ├── generate_describe_focus.py    # P0-4：AI生成影像描述
│   ├── detect_cTcN_leak.py           # P0-4：泄露检测
